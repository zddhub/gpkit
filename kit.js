...
const sql = `
{
  repository(owner: "${Deno.env.get("GITHUB_OWNER")}", name: "${Deno.env.get("GITHUB_PROJECT_NAME")}") {
    milestones(first: 20) {
      nodes {
        title
...
const results = milestones.map(milestone => {
  ...
  const events = issues.map(issue => {
    ...
    const doingAt = timelines.filter(timeline => timeline.projectColumnName === DOING_COLUMN_NAME).shift()?.createdAt || ""
    const doneAt = timelines.filter(timeline => timeline.projectColumnName === DONE_COLUMN_NAME).pop().createdAt || ""
    return {
      title: issue.title,
      url: issue.url,
      doingAt,
...
let csv = new CsvFile(await Deno.open(OUTPUT_FILE, {read: true, write: true, create: true, truncate: true}))
csv.writeRecordSync(["Title", "Url", "Doing At", "Done At", "Circle Time (days)", "Milestone"])


results.map(result =>  {
  result.events.map(event => {
    csv.writeRecordSync([event.title, event.url, event.doingAt, event.doneAt, event.cycleTime, result.title])
  })
})
